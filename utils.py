...
def load_test_data(image_path, fine_size=256):
    ...
    img = sktransf.resize(img, [fine_size, fine_size * 2])
    img = img / 127.5 - 1
    return img
...
def load_train_data(image_path, load_size=286, fine_size=256, is_testing=False):
    ...
    if not is_testing:
        img_A = sktransf.resize(img_A, [load_size, load_size * 2])
        img_B = sktransf.resize(img_B, [load_size, load_size * 2])
        ...
        img_A = sktransf.resize(img_A, [fine_size, fine_size * 2])
        img_B = sktransf.resize(img_B, [fine_size, fine_size * 2])
...
def get_image(image_path, image_size, is_crop=True, resize_w=64, is_grayscale=False):
    return transform(
        imageio.imread(image_path, is_grayscale), image_size, is_crop, resize_w
    )
...
def imread(path, is_grayscale=False):
    if is_grayscale:
        return scipy.misc.imageio.imread(path, flatten=True).astype(np.float)
    else:
        return scipy.misc.imageio.imread(path, mode="RGB").astype(np.float)
...
def center_crop(x, crop_h, crop_w, resize_h=64, resize_w=64):
    ...
    return sktransf.resize(x[j : j + crop_h, i : i + crop_w], [resize_h, resize_w])
